...
try {
	...
	(status, remark) = CustomKeywords.'myPac.StaffKeywords_COD.findOrder'(order_id, status_id)
	if (status.equals('Fail')) {
		return CustomKeywords.'myPac.StaffKeywords_COD.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
	}
	
	(status, remark, size) = CustomKeywords.'myPac.StaffKeywords_COD.checkTotalProducts'(flow_type, size, status_id, statusCheck)
	if (status.equals('Fail')) {
		return CustomKeywords.'myPac.StaffKeywords_COD.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
	}
	...
	for(int i = 0; i < size; i++) {
		...
		(status, remark, countQty, countTotalPrice) = CustomKeywords.'myPac.StaffKeywords_COD.checkEachProduct'(productName[i], productQty[i], productUnitPrice[i], countQty, countTotalPrice, statusProduct, status_id)
		if (status.equals('Fail')) {
			return CustomKeywords.'myPac.StaffKeywords_COD.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
		}
	...
	(status, remark) = CustomKeywords.'myPac.StaffKeywords_COD.checkAllProducts'(countTotalPrice, total_price, countQty, status_id)
	if (status.equals('Fail')) {
		return CustomKeywords.'myPac.StaffKeywords_COD.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
	}
	...
	switch (flow_type) {
		//new order
		case '1' :
			(status, remark) = CustomKeywords.'myPac.StaffKeywords_COD.cancelBtn'(flow_type, status_id)
			if (status.equals('Fail')) {
				return CustomKeywords.'myPac.StaffKeywords_COD.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
			}
			status_id = 6
...
//	status_id = 2
	...
	if (flow_type != '1') {
		...
		(qty, unitPrice, countQty, countTotalPrice, statusProduct, size, price) = CustomKeywords.'myPac.StaffKeywords_COD.setDefault'(total_product, flow_type, statusCheck)
			...
			default :
				switch (flow_type) {
					case '5' :
						...
						qty = edit_qty
						CustomKeywords.'myPac.StaffKeywords_COD.editQty'(edit_product, qty, oldQty)
						break
					case '6' :
						println ('Add product')
						qty = edit_qty
						CustomKeywords.'myPac.StaffKeywords_COD.addProduct'(edit_product, qty)
						break
					case '7' :
						println ('Delete product')
						CustomKeywords.'myPac.StaffKeywords_COD.deleteProduct'(edit_product)
						break
				}
				...
				switch (flow_type) {
					case '5'..'7' :
						...
						for(int i = 0; i < products.size(); i++) {
							for (int j = 0; j < productName.size(); j++) {
								if (products.get(i).getText() == productName[j]) {
									...
									(status, remark, countQty, countTotalPrice) = CustomKeywords.'myPac.StaffKeywords_COD.checkEachProduct'(productName[j], qty, productUnitPrice[j], countQty, countTotalPrice, statusProduct, status_id)
									if (status.equals('Fail')) {
										return CustomKeywords.'myPac.StaffKeywords_COD.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
									}
								}
						...
						switch (flow_type) {
							case '6' :
								qty = edit_qty
								(status, remark, countQty, countTotalPrice) = CustomKeywords.'myPac.StaffKeywords_COD.checkEachProduct'(edit_product, qty, edit_unit_price, countQty, countTotalPrice, statusProduct, status_id)
								if (status.equals('Fail')) {
									return CustomKeywords.'myPac.StaffKeywords_COD.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
								}
								break
						...
						println ('countQty : ' + countQty)
						println ('countTotalPrice : ' + countTotalPrice)
						(status, remark) = CustomKeywords.'myPac.StaffKeywords_COD.checkAllProducts'(countTotalPrice, edit_total_price, countQty, status_id)
						if (status.equals('Fail')) {
							return CustomKeywords.'myPac.StaffKeywords_COD.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
						}
						break
...
//	Mobile.delay(8)
	...
	(status, remark) = CustomKeywords.'myPac.StaffKeywords_COD.checkStatusId'(status_id)
