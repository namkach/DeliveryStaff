...
try {
	...
	for(int i = 0; i < products.size(); i++) {
		...
		(countQty, countTotalPrice) = CustomKeywords.'myPac.StaffKeywords.checkProduct'(productName[i], Integer.parseInt(productQty[i]), productUnitPrice[i], countQty, countTotalPrice, statusProduct)
	...
	switch (flow_type) {
		//new order
		case '1' :
			statusOrder = CustomKeywords.'myPac.StaffKeywords.CancelBtn'(flow_type)
			status_id = '6'
		...
		//new order
		case '0'..'7' :
			(statusOrder, status_id, paymentType, deliveryType) = CustomKeywords.'myPac.StaffKeywords.ConfirmBtn'(status_id, paymentType, apkType)
	...
	if (flow_type != '1') {
		...
		checkOrder2 = CustomKeywords.'myPac.StaffKeywords.FindOrder'(order_id)
		...
		if (flow_type == '2') {
			statusOrder2 = CustomKeywords.'myPac.StaffKeywords.CancelBtn'(flow_type)
			status_id = '6'
			KeywordUtil.markPassed('Cancel : Pass')
		} else {
			if (flow_type == '5') {
			...
			if (flow_type == '5' || flow_type == '6' || flow_type == '7') {
				...
				for(int i = 0; i < size; i++) {
					...
					(countQty, countTotalPrice) = CustomKeywords.'myPac.StaffKeywords.checkProduct'(productName[i], qty, productUnitPrice[i], countQty, countTotalPrice, statusProduct)
				}
				if (flow_type == '6') {
					qty = Integer.parseInt(edit_qty)
					(countQty, countTotalPrice) = CustomKeywords.'myPac.StaffKeywords.checkProduct'(edit_product, qty, edit_unit_price, countQty, countTotalPrice, statusProduct)
				}
				println ('countQty : ' + countQty)
				println ('countTotalPrice : ' + countTotalPrice)
			...
			(statusOrder2, status_id, paymentType, deliveryType) = CustomKeywords.'myPac.StaffKeywords.ConfirmBtn'(status_id, paymentType, apkType)
	...
	CustomKeywords.'myPac.writeExcel.writeStaff'(order_id, flow_type, deliveryType, payment_type, status, remark)
}
catch (Exception e) {
