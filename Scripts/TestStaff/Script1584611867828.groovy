...
//def path = 'D:\\Users\\sunitakac\\Desktop\\apk\\staff_uat_1.4.6j_RIDER_29042020.apk'
...
try {
	...
	CustomKeywords.'myPac.StaffKeywords.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
}
...
try {
	...
	if (status.equals('Fail')) {
		return CustomKeywords.'myPac.StaffKeywords.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
	}
...
//	status_id = 2
	...
	if (flow_type != '1') {
//		Mobile.tap(findTestObject('Staff/PackingTab'), 90)
						...
						for(int i = 0; i < products.size(); i++) {
							for (int j = 0; j < productName.size(); j++) {
								if (products.get(i).getText() == productName[j]) {
									...
									(status, remark, countQty, countTotalPrice) = CustomKeywords.'myPac.StaffKeywords.checkEachProduct'(productName[j], qty, productUnitPrice[j], countQty, countTotalPrice, statusProduct, status_id)
									if (status.equals('Fail')) {
										return CustomKeywords.'myPac.StaffKeywords.writeStaff'(order_id, flow_type, delivery_type, payment_type, status, remark)
									}
								}
...
//	Mobile.tap(findTestObject('Staff/ProcessingTab'), 90)
...
//	Mobile.tap(findTestObject('Staff/ProcessedTab'), 90)
